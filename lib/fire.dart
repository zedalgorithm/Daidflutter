...
class ReportFireEmergencyPage extends StatefulWidget {
  ...
  @override
  _ReportFireEmergencyPageState createState() =>
      _ReportFireEmergencyPageState();
}
...
class _ReportFireEmergencyPageState extends State<ReportFireEmergencyPage> {
  ...
  LatLng _currentPosition =
      const LatLng(11.6072, 125.4353); // Variable to hold the current position
  ...
  String timeNow = DateFormat('HH:mm:ss')
      .format(DateTime.now()); // Variable to hold the current time
  ...
  Future<void> _getCurrentLocation() async {
    ...
    setState(() {
      _currentPosition = LatLng(
          position.latitude, position.longitude); // Create LatLng from Position
      _markers.add(
        Marker(
          markerId: const MarkerId('current_location'),
          position: _currentPosition,
          infoWindow: const InfoWindow(
              title: 'You are here'), // Info window for the marker
          draggable: true, // Make the marker draggable
          onDragEnd: (newPosition) {
            setState(() {
              _currentPosition =
                  newPosition; // Update the current position when the marker is dragged
              _getAddressFromLatLng(
                  newPosition); // Fetch the address from the new position
              mapController.animateCamera(
                CameraUpdate.newLatLng(
                    newPosition), // Animate camera to new position
              );
            });
          },
        ),
      ...
      _getAddressFromLatLng(
          _currentPosition); // Fetch the address from the current position
    });
  }
  ...
  Future<void> _getAddressFromLatLng(LatLng position) async {
    ...
    if (placemarks.isNotEmpty) {
      ...
      setState(() {
        _currentAddress =
            '${placemarks[0].street}, ${placemarks[0].subLocality ?? ''}, ${placemarks[0].locality}, ${placemarks[0].subAdministrativeArea}, ${placemarks[0].country}'; // Format the address
        _addressController.text =
            _currentAddress; // Update the TextEditingController
      });
    }
  }
  ...
  Future<void> _getUserAddress() async {
    ...
    try {
      ...
      String reverseGeocodeUrl =
          'https://maps.googleapis.com/maps/api/geocode/json?latlng=${position.latitude},${position.longitude}&key=AIzaSyAYQBD5gUFDhRQNDxd_bx-eesznMxijX8k';
  ...
  Future<void> _fetchUserDetails() async {
    ...
    if (user != null) {
      ...
      setState(() {
        userDetails =
            userDoc.data() as Map<String, dynamic>; // Store user details
      });
    }
  }
  ...
  Future<void> _sendReport() async {
    ...
    Map<String, dynamic> reportData = {
      ...
      'reportedFor': selectedButton == 0
          ? 'For Myself'
          : selectedButton == 1
              ? 'For Someone'
              : 'Unknown',
      'requestDate': DateFormat('MM/dd/yyyy')
          .format(requestDate), // Format to date/month/year
      'timeNow': DateFormat('hh:mm a')
          .format(DateTime.now()), // Format to 12-hour time
      'userDetails': {
        'address': userDetails?[
            "address"], // Assuming userAddress holds the user's address
        ...
        'phoneNumber': userDetails?[
            'phoneNumber'], // Extract phone number from userDetails
        'photo': userDetails?['photo'], // Extract photo URL from userDetails
        'religion':
            userDetails?['religion'], // Extract religion from userDetails
        'stability':
            userDetails?['stability'], // Extract stability from userDetails
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          ...
          backgroundColor: const Color(0xFFF1811A), // AppBar background
          title: RichText(
            text: TextSpan(
              children: [
                WidgetSpan(
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.white, // White background for emoji
                      borderRadius:
                    ...
                    child: Text(
                      ...
                      style: TextStyle(
                        ...
                        color: const Color(
                            0xFFF1811A), // Orange emoji (or use black for contrast)
                      ),
                    ),
                  ),
                ),
                TextSpan(
                  ...
                  style: TextStyle(
                    fontSize: textScale.scale(18.0),
                    fontWeight: FontWeight.bold,
                    color: Colors.white, // Orange text
                  ),
                ),
              ],
            ),
              ...
              child: Stack(
                children: [
                  GoogleMap(
                    ...
                    myLocationEnabled:
                        false, // Disable user location blue dot
                      ...
                      children: [
                        ...
                        FloatingActionButton(
                              ...
                              setState(() {
                                ...
                                _markers.add(
                                  Marker(
                                    ...
                                    infoWindow: const InfoWindow(
                                        title:
                                            'You are here'), // Info window for the marker
                                    draggable:
                                        true, // Make the marker draggable
                                    onDragEnd: (newPosition) {
                                      setState(() {
                                        _currentPosition =
                                            newPosition; // Update the current position when the marker is dragged
                          ...
                          backgroundColor: Colors
                              .white, // Set to true to make the button smaller
                          child: const Icon(
                            Icons.my_location,
                  ...
                  Align(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: const [
                          BoxShadow(
                            color: Color(0x80202020), // Grey with 50% opacity
                            spreadRadius: -15,
                            blurRadius: 1,
                            offset: Offset(4, 10),
                          ),
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          contentPadding:
                              const EdgeInsets.all(8), // Adjust left padding
                          border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(60),
                              borderSide: const BorderSide(
                                  color:   Color(0xFFF1811A), width: 0.5)),
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: selectedButton == 0 ? const Color(0xFFF1811A) : Colors.white,
                    foregroundColor: selectedButton == 0 ? Colors.white : Color(0xFFF1811A),
                    padding:
                        EdgeInsets.symmetric(vertical: screenHeight * 0.02),
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(8),
                      side: const BorderSide(color: Color(0xFFF1811A), width: 0.5)
                    ),
                  ),
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    backgroundColor: selectedButton == 1 ? Color(0xFFF1811A) : Colors.white,
                    foregroundColor: selectedButton == 1 ? Colors.white : Color(0xFFF1811A),
                    padding:
                        EdgeInsets.symmetric(vertical: screenHeight * 0.02),
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(8),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: selectedEmergencyType == 0 ? Color(0xFFF1811A) : Colors.white,
              border: Border.all(
                color:
                    selectedEmergencyType == 0 // Check if "Crime" is selected
              ...
              children: [
                Padding(
                  ...
                  child: Icon(
                    ...
                    color: selectedEmergencyType == 0 ? Colors.white : const Color(0xFFF1811A),
                    size: 25,
                  ),
                ),
                            ...
                            if (pickedFile != null) {
                              setState(() {
                                _image = File(pickedFile
                                    .path); // Update the image variable
                              });
                            }
                          },
                          style: ElevatedButton.styleFrom(
